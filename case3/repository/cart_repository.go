...
func (cr *cartRepository) GetChartByUserIDAndProductID(ctx context.Context, userId uuid.UUID, productId int) (*model.Cart, error) {
	q := fmt.Sprintf(cr.QueryBasicSelect + ` WHERE user_id = $1 AND product_id = $2 AND used_at IS NULL LIMIT 1`)
...
func (cr *cartRepository) CheckAvailabilityCartList(ctx context.Context, userID uuid.UUID) ([]model.Cart, error) {
	q := `SELECT 
		...
		FROM carts c LEFT JOIN products p ON c.product_id = p.id WHERE user_id = $1 AND used_at IS NULL`
...
func (cr *cartRepository) GetChartByIDs(ctx context.Context, charIds []int) ([]model.Cart, error) {
	q := fmt.Sprintf(cr.QueryBasicSelect + ` WHERE id = ANY($1) AND used_at IS NULL LIMIT 1`)
